...
double util::variance(const std::vector<double>& x) {
...
double util::geo_haversine_distance(const double& lat1d, const double& lon1d,
...
double util::euclidean_distance(const double& x1, const double& y1,
...
util::generate_circle(const std::size_t& points, const double& radius,
...
std::string util::tid_to_string(const std::thread::id& tid) {
    std::stringstream ss;
    ss << tid;
    return ss.str();
}

...
Eigen::Matrix2d calci::info_mat_gps_xy()
{
    ...
    inv(0, 0) = 1 / std::max(utils::variance(_x_gps), _min_value);
    inv(1, 1) = 1 / std::max(utils::variance(_y_gps), _min_value);
...
Eigen::Matrix2d calci::info_mat_vo_xy()
{
    ...
    inv(0, 0) = 1 / std::max(utils::variance(_x_vo), _min_value);
    inv(1, 1) = 1 / std::max(utils::variance(_y_vo), _min_value);
...
Eigen::Matrix<double, 6, 6> calci::info_mat_vo_se3()
{
    ...
    inv(0, 0) = 1 / std::max(utils::variance(_x_vo), _min_value);
    inv(1, 1) = 1 / std::max(utils::variance(_y_vo), _min_value);
    inv(2, 2) = 1 / std::max(utils::variance(_z_vo), _min_value);
    inv(3, 3) = 1 / std::max(utils::variance(_roll), _min_value);
    inv(4, 4) = 1 / std::max(utils::variance(_pitch), _min_value);
    inv(5, 5) = 1 / std::max(utils::variance(_yaw), _min_value);

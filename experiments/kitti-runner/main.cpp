...
using slam_input_t = std::tuple<ORB_SLAM2::ext::time_point_t,
                                ORB_SLAM2::ext::image_t,
                                ORB_SLAM2::ext::tsr_info_opt_t,
                                ORB_SLAM2::ext::pos_info_opt_t>;
...
void feed_images(ORB_SLAM2::System &slam, const fs::path &image_folder)
{
    ...
    {
        auto image = cv::imread(image_file.generic_string(),
                  ...
                  const fs::path &gt)
{
    ...
    for (std::size_t i = 0; i < image_files.size(); ++i)
    {
        ...
        auto input = std::make_tuple(i, image, boost::none, posi);
        track(slam, input);
    }
}

void run_slam(const std::string& option, const fs::path &voc, const fs::path &settings,
...
void run(int argc, char **argv)
{
    ...
    if (argc != 6)
        throw std::invalid_argument(
            "usage: kitti_runner option(gps|no-gps) vocabulary settings image_folder "
            "sensor_source(groundtruth)");
    ...
    run_slam(option, voc, settings, image_folder, sensor_source);
}

...
Eigen::Vector3d gps_adder::rotate(const Eigen::Vector3d& point, const ext::orientation_t& rot)
{
...
Eigen::Vector3d gps_adder::to_local(pos_info_opt_t& curr_gps)
{
    ...
    if(gps_source::oxts == _gps_source)
    {
        ...
        auto rotated_point = rotate(
            Eigen::Vector3d(local_geo.x, local_geo.y, local_geo.z), 
            initial_rot);
...
double gps_adder::distance(pos_info_opt_t& last_gps, pos_info_opt_t& curr_gps)
{
    ...
    if(gps_source::oxts == _gps_source)
    {
        ret = util::geo_euclidean_distance(
            last_gps.get().pos.x(), last_gps.get().pos.y(),
            curr_gps.get().pos.x(), curr_gps.get().pos.y());
    }
    else if(gps_source::ground_truth == _gps_source)
    {
        ret = util::euclidean_distance(
            last_gps.get().pos.x(), last_gps.get().pos.y(), last_gps.get().pos.z(),
            curr_gps.get().pos.x(), curr_gps.get().pos.y(), curr_gps.get().pos.z());
    }

    return ret;
}

Eigen::Matrix<double, 1, 1> gps_adder::calculate_inf_of_dist_edge(
...
void gps_adder::add_gps_dist_edge(
    g2o::VertexSE3Expmap* last_cam_with_gps,
...
)
{
    ...
    else if(gps_option::binary_gps_edge == _gps_ba_fuse_option)
    {
        if(nullptr != last_cam_with_gps && last_gps)
            add_gps_dist_edge(
                last_cam_with_gps, curr_cam_with_gps, last_gps, curr_gps);
    }
...
void gps_adder::add_gps_to_pose_optimization(
...
void gps_adder::add_gps_to_ba_for_local_kf(
    g2o::VertexSE3Expmap * vSE3,
    pos_info_opt_t curr_gps
)
{
...
void gps_adder::add_gps_to_ba_for_fixed_kf(
    g2o::VertexSE3Expmap * vSE3,
    pos_info_opt_t curr_gps
)
{
